...
def execute_benchmark(bulkload_percent, method)
  system "./release/benchmark_merge_query \
...
def analyze_mergeperformance(bulkload_percent, method)
  return unless File.exists? outfile(bulkload_percent, method)
  content = File.readlines(outfile(bulkload_percent, method)).reverse
  runtime_ms_str = content.select{ |line| line =~ /Merge method \d runtime/ }.first.chomp
  runtime_ms = runtime_ms_str.match(/(\d+)$/)[1]
  puts [
    bulkload_percent,
    method,
    runtime_ms,
...
def execute
  ...
  bulkload_percents = [0.6, 0.7, 0.8, 0.9]
  methods = [0, 1]
  ...
  puts "bulkload_percent;method;runtime_ms"
  methods.each do |method|
    bulkload_percents.each do |bulkload_percent|
      analyze_mergeperformance(bulkload_percent, method)
    end
  end
